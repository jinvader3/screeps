...
class Socket(object):
    ...
    def unsubscribe_user(self, watchpoint):
        self.unsubscribe('user:' + self.user_id + '/' + watchpoint)

    def unsubscribe(self, watchpoint):
    ...
    def process_whole_log(self, ws, messages, shard='shard0'):
        pass

    def process_log_seperator(self, ws, shared='shard0'):
        pass
    ...
    def process_room_user(self, ws, user_id, user, room, shard):
        pass
    ...
    def on_message(self, ws, message):
        ...
        try:
            ...
            if data[0].endswith('console'):
                ...
                if 'messages' in data[1]:
                    ...
                    if 'log' in data[1]['messages']:
                        self.process_whole_log(
                            ws, data[1]['messages']['log'], shard
                        )
                        for line in data[1]['messages']['log']:
                            self.process_log(ws, line, shard)
                        self.process_log_seperator(ws, shard)
            ...
            if data[0].startswith('room:'):
                shard = data[0][data[0].find(':')+1:data[0].find('/')]
                room = data[0][data[0].find('/')+1:]
                objs = data[1]['objects']
                for obj_id in objs:
                  self.process_room_object(ws, obj_id, objs[obj_id], room, shard)
                users = data[1]['users']
                for user_id in users:
                  self.process_room_user(ws, user_id, users[user_id], room, shard)
